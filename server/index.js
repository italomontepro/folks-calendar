...
app.post(base+'/events',edit,(req,res) => {
...
app.post(base+'/hooks',admin,(req,res) => {
  const hook = {...hookInput(req.body),id:randomUUID(),secret:randomBytes(32).toString('hex'),workspaceId:req.workspaceId};
  put('hooks',hook,req.workspaceId);res.status(201).json(hook);
});
app.put(base+'/hooks/:id',admin,(req,res) => {
  const old = get('hooks',req.params.id,req.workspaceId);
  if (!old) fail(404,'Automação não encontrada.');
  const hook = {...old,...hookInput(req.body)};
  transaction(() => {
    put('hooks',hook,req.workspaceId);
    if (!hook.enabled) db.prepare("UPDATE jobs SET status='cancelled' WHERE hook=? AND workspace_id=? AND status='pending'").run(hook.id,req.workspaceId);
  });
  res.json(hook);
});
...
app.get(base+'/deliveries',admin,(req,res) => res.json(db.prepare('SELECT id,hook,attempts,status,error,created,payload FROM jobs WHERE workspace_id=? ORDER BY created DESC LIMIT 100').all(req.workspaceId).map(j=>({...j,type:JSON.parse(j.payload).type,payload:undefined}))));
